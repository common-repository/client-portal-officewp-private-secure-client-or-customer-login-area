...
use wpo\list_table\List_Table_Members;
$ListTable = new List_Table_Members();
...
<div class="wpo_admin_members wpo_admin_wrapper">
...
<script type="text/javascript">
    jQuery( document ).ready( function() {
        ...
        //create member layer
        body.on( 'click', '.wpo_admin_members .wpo_create_member:not(.wpo_disabled)', function(e) {
            var current_role = jQuery(this).parents('.wpo_admin_members').find('.wpo_list_table_filters_line .wpo_current').parents('li').attr('class');
            var table_uniqueid = jQuery(this).parents('.wpo_admin_members').find('.wpo_list_table_wrapper').data('id');

            jQuery.pulllayer({
                ajax_url        :  '<?php echo WO()->get_ajax_route( 'wpo\form\Member_Form', 'display_add' ) ?>',
                ajax_data       : 'role=' + current_role,
                object          : this,
                custom_data     : {
        ...
        //edit member layer
        body.on( 'click', '.wpo_admin_members .wpo_edit a', function(e) {
        ...
        //view member layer
        body.on( 'click', '.wpo_admin_members .wpo_view a', function(e) {
            var id = jQuery( this ).data( 'member_id' );
            var current_role = jQuery('.wpo_list_table_filters_line').find('.wpo_current').parents('li').attr('class');
            var table_uniqueid = jQuery(this).parents('.wpo_list_table_wrapper').data('id');
        ...
        body.on( 'click', '.wpo_admin_members .wpo_archive a', function(e) {
            var obj = jQuery(this);
            var table_uniqueid = jQuery(this).parents('.wpo_list_table_wrapper').data('id');

            jQuery.wpo_confirm({
                message : '<?php _e( 'Are you sure archive this Member?', WP_OFFICE_TEXT_DOMAIN ) ?>',
                onYes: function() {
                    jQuery.ajax({
                        type: "POST",
                        url: '<?php echo WO()->get_ajax_route( get_class( $ListTable ), 'archive_members' ) ?>',
                        ...
                        success: function( data ) {
                            ...
                            jQuery( this ).wpo_notice({
                                message : data.status ? '<?php _e( 'Member was Archived!', WP_OFFICE_TEXT_DOMAIN ) ?>' : data.message,
        ...
        body.on( 'click', '.wpo_admin_members .wpo_delete a', function(e) {
            var obj = jQuery(this);
            var table_uniqueid = jQuery(this).parents('.wpo_list_table_wrapper').data('id');
        ...
        jQuery( document ).on( "wpo_list_table_data_loaded", function( event, data ) {
            if( data.can_create ) {
                jQuery('.wpo_create_member').removeClass('wpo_disabled');
            } else {
                jQuery('.wpo_create_member').addClass('wpo_disabled');
            }
        });
    });
</script>
